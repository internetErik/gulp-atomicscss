var emmetWrapper = require("./emmetWrapper");
...
function Molecule(classes) {
	...
	function stripMoleculeName() {
		molecule.className = classes.filter(name => name[0] === '_')[0];
	}
	...
	function getPseudoClassNames() {
		molecule.atoms.map(function (atom){ 
			if(atom.pseudo !== '' && !molecule.pseudoTypes.includes(atom.pseudo)){
	...
	function getMediaQueries() {
		molecule.atoms.map(function (atom){ 
			if(atom.mediaQuery !== '' && !molecule.mediaQueries.includes(atom.mediaQuery)){
